...
/* 1. gets data from the database,
   ...
   3. extracts the pieces it needs,
   4. decides which sections should appear,
   5. assigns alternating backgrounds,
   6. renders the sections in order.
   two different kinds of "sections" in application
    1. Database sections = represent content/ configuration and from: getWebsiteData()
    2. React sections = represent presentation/UI 
...
export default async function PhysicianPage(_props: PhysicianPageProps) {
  ...
  // First validation: to make sure website data is valid = success
  ...
  // rendering configuration: to create a list of sections that are actually enabled so that background colors can be alternated among sections that actually render
  ...
  // The Fragment is essentially invisible in the resulting HTML and let you group the result without adding another HTML element such as <div></div>
  ...
  // final JSX is the actual page structure
  return (
